...
/**
  ...
  */
case class Ping(id: Int, date: Timestamp, productId: Int, license: String, user: String, ip: String, responseId: Option[Int]) {
  def queryProduct()(implicit ec: ExecutionContext, productsDAO: ProductsDAO): Future[Product] =
    ...
    productsDAO.findById(productId).map(_.get)
